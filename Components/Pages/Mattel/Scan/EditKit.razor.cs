...
public partial class EditKit : IDisposable
{
    ...
    [Parameter]
    public EventCallback<MattelClass.Sheet> OnKitEdited { get; set; }

    [Parameter]
    public MattelClass.Sheet EditKitData { get; set; } = new();

    public string taskName = "EditKit";
    private MessageLevel settingLevel => DebugParameters.MsgLvl_ScanEditKit;
    ...
    private MattelClass.Sheet list = new();
    private string? CurrentKit;
    private string? NewKit;
    private string ConfirmButtonColor => string.IsNullOrWhiteSpace(EditKitData.KitNumber)
    ...
    protected override void OnInitialized()
    {
        ...
        CurrentKit = list.KitNumber;
        NewKit = list.KitNumber;
        base.OnInitialized();
    }
    ...
    {
        EditKitData.KitNumber = e.Value?.ToString()!;
        await InvokeAsync(StateHasChanged);
    }
    ...
    {
        await OnKitEdited.InvokeAsync(EditKitData);
        await CloseModal();
    }
}