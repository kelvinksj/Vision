...
public partial class EditOrderKit : IDisposable
{
    [Parameter]
    public PopEditData PopEditData { get; set; } = new();

    public string taskName = "EditOrderKit";
    private MessageLevel settingLevel => DebugParameters.MsgLvl_EditOrderKit;
    ...
    private bool confirmDisable => string.IsNullOrWhiteSpace(PopEditData.NewValue);
    private string confirmColor => string.IsNullOrWhiteSpace(PopEditData.NewValue)
    ...
    private async void CheckInput(ChangeEventArgs e)
    {
        ...
        try
        {
            PopEditData.NewValue = e.Value?.ToString() ?? string.Empty;
            await InvokeAsync(StateHasChanged);
        }
        catch (Exception ex)
        {
    ...
    private async void CloseModal()
    {
        ...
        try
        {
            if (PopEditData != null && PopEditData.EventNok.HasDelegate)
            {
                PopEditData.IsVisible = false;
                await PopEditData.EventNok.InvokeAsync();
            }
            else
    ...
    private async Task Confirm()
    {
        ...
        try
        {
            if (PopEditData != null && PopEditData.EventOk.HasDelegate)
            {
                await PopEditData.EventOk.InvokeAsync();
            }
            else
