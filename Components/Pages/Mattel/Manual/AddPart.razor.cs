...
public partial class AddPart : IDisposable
{
    ...
    [Parameter]
    public EventCallback<MattelClass.SheetData> OnPartAdded { get; set; }

    public string taskName = "AddPart";
    private MessageLevel settingLevel => DebugParameters.MsgLvl_AddPart;
    ...
    private async void CheckInput(ChangeEventArgs e)
    {
        ...
        list.PartNumber = e.Value?.ToString()!.Replace(" ", string.Empty) ?? string.Empty;
        await InvokeAsync(StateHasChanged);
    }
