...
public partial class CPCNew : IDisposable
{
    private string taskName = "CPCNew";
    ...
    private MsgGrpList mgList = new();
    private string areaCode = "2300";
    private string areaCode1 = "2301";
    private string areaCode2 = "2302";
    private List<MattelClass.LineData> lineDatas = new();
    ...
    private MattelClass.CPCSetup cPCSetup = new();
    ...
    protected override async Task OnAfterRenderAsync(bool FirstRender)
    {
        if (FirstRender && urlPage == taskName)
        {
            ...
            try
            {
                ...
                ContainerService.SubscribeContainer(ContainerUpdate, areaCode1);
                ContainerService.SubscribeContainer(ContainerUpdate, areaCode2);
                AddMessageGroup(areaCode1);
                AddMessageGroup(areaCode2);
                TaskManagerService?.SubscribeMessage(MsgUpdate, areaCode1);
                TaskManagerService?.SubscribeMessage(MsgUpdate, areaCode2);
            }
            catch (Exception ex)
            {
                CommonLib.DisplayConsole(
        ...
        if (!disposed)
        {
            ...
            // Cleanup managed resources
            {
                try
                {
                    ...
                    try
                    {
                        ContainerService.UnsubscribeContainer(ContainerUpdate, areaCode1);
                        ContainerService.UnsubscribeContainer(ContainerUpdate, areaCode2);
                        RemoveMessageGroup(areaCode1);
                        RemoveMessageGroup(areaCode2);
                        TaskManagerService?.UnsubscribeMessage(MsgUpdate, areaCode1);
                        TaskManagerService?.UnsubscribeMessage(MsgUpdate, areaCode2);
                    }
                    catch (Exception ex)
                    {
                        CommonLib.DisplayConsole(
    ...
    // Update all the necessary variable or task for the page when URL change
    {
        ...
        try
        {
            ...
            lineData = lineDatas.Where(
                n => n.Point[..4] == areaCode1
                    || n.Point[..4] == areaCode2
            ).ToList();
            cPCSettings = MattelService.GetCPCSettings();
            locSettings = cPCSettings.LocationSettings.Where(
                n => n.LocationID[..4] == areaCode1
                    || n.LocationID[..4] == areaCode2
            ).ToList();
            oldLocSettings = locSettings.DeepClone();

            cPCSetup.Header1 = "Lane 1";
            cPCSetup.Header2 = "Lane 2";
            cPCSetup.IsLineEdit = false;
    ...
    private void AddMessageGroup(string AreaID)
    {
        CommonLib.DisplayConsole(
              taskName,
              $"AddToMessageGroup triggered",
              settingLevel,
              MessageLevel.Information
           );
        ...
        try
        {
            var findArea = mgList?.List.FirstOrDefault(
                n => n.AreaID == AreaID
            ) ?? null!;
    ...
    private void RemoveMessageGroup(string AreaID)
    {
        CommonLib.DisplayConsole(
              taskName,
              $"RemoveToMessageGroup triggered",
              settingLevel,
              MessageLevel.Information
           );
    ...
    private async void MsgUpdate(List<HttpClass.UIMessage> UIMessages)
    {
        ...
        try
        {
            if (UIMessages != null && mgList != null)
            {
                var findArea = mgList.List.FirstOrDefault(
                    n => n.AreaID == UIMessages.FirstOrDefault()?.AreaID
                ) ?? null!;

                if (findArea != null)
                    findArea.List = UIMessages;

                var count = 1;
                mList = new();

                foreach (var area in mgList.List)
                {
                    if (area.List.Count > 0)
                    {
                        foreach (var msg in area.List)
                        {
    ...
    private async void OnClickLine(int ButtonNumber)
    {
        CommonLib.DisplayConsole(
           taskName,
           $"OnClickLine triggered",
           settingLevel,
           MessageLevel.Information
        );
        ...
        try
        {
            ...
            switch (ButtonNumber)
            {
                ...
                case 2:
                    cPCSetup.Header1 = "Lane 3";
                    areaCode1 = "2303";
                    cPCSetup.Header2 = "Lane 4";
                    areaCode2 = "2304";
                    break;

                case 3:
                    cPCSetup.Header1 = "Lane 5";
                    areaCode1 = "2305";
                    cPCSetup.Header2 = "Lane 6";
                    areaCode2 = "2306";
                    break;
            }
