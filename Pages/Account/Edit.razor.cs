...
public partial class Edit : IDisposable
{
    ...
    [Parameter]
    public EventCallback<UserAccount> OnUserEdited { get; set; }

    private string taskName = "Edit";
    private MessageLevel settingLevel => DebugParameters.MsgLvl_AccEdit;
    ...
    private bool isFirstNameDisable = true;
    private bool isLastNameDisable = true;
    private bool isRoleDisable = true;
    ...
    (
        string.Equals(
            currentUserRole,
            nameof(Role.Engineer),
            StringComparison.OrdinalIgnoreCase
        )
        || string.Equals(
            currentUserRole,
            nameof(Role.Supervisor),
            StringComparison.OrdinalIgnoreCase
        )
        || string.Equals(
            currentUserRole,
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        {
            user = UserAccountService.GetByUserId(userID)!;
            currentUserRole = authClaims.FirstOrDefault(c => c.Type == ClaimTypes.Role)!.Value;

            if (currentUserRole == nameof(Role.Administrator))
                if (currentUserName != user.UserName)
                {
                    isRoleDisable = false;
                }

            if (user.UserName != "Admin" && user.UserName != "Administrator")
            {
    ...
    {
        await OnUserEdited.InvokeAsync(user); // Notify the parent component
    }
